...
namespace houseAgency.mothedCls
{
    class clstypeMethod
    {
        ...
        public string insert_table(clstype cf)
        {
            try
            {
                con.ConDatabase();
                SqlCommand cmd = new SqlCommand("proc_type_insert", con.conn);
                ...
                SqlParameter[] prams =
                {
                		...
                		new SqlParameter("@type_remark",  SqlDbType.VarChar, 50),
        ...
        public bool update_table(clstype cf)
        {
            try
            {
                con.ConDatabase();
                SqlCommand cmd = new SqlCommand("proc_type_update", con.conn);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Connection.Open();
                SqlParameter[] prams =
                {
        ...
        public bool delete_table(clstype cf)
        {
            try
            {
                con.ConDatabase();
                SqlCommand cmd = new SqlCommand("proc_type_delete", con.conn);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Connection.Open();
                SqlParameter[] prams =
                {
